...
This approach was motivated by the work by \cite{hastings1970monte} who proposes an approximation for the variance of ratio of two random variables. However the formula provided by \cite{hastings1970monte} contains two typos: on page 8, in the formula for variance of the ratio, $Z$ is missing the "bar" symbol and computed variance $s_{\bar{Z}}$ is missing the second power. The missing second power made caused me to mistake it for the standard deviation instead of variance, which was followed by weird simulation results. So the on of the contributions of this report is providing the typo free formula and adding the derivation of the result.
