...
This approach was motivated by the work by \cite{hastings1970monte} who proposes an approximation for the variance of ratio of two random variables. However the formula provided by \cite{hastings1970monte} contains two typos: on page 8, in the formula for variance of the ratio, $Z$ is missing the "bar" symbol and computed variance $s_{\bar{Z}}$ is missing the second power. The missing second power caused the author of this report to mistake it for the standard deviation instead of variance, which was followed by weird simulation results. 
...
Figure \ref{fig:simres_low_n} runs the similar experiments, but $n$ is now either 5 or 10. Not surprisingly, we see more dispersion on the standard error estimates. One pattern seems to be that when the R2 estimate is small (darker dots) compared to the standard error, we start to bias in the Taylor approximation.
